...
$ovabrw_dropoff_date    = isset( $_GET['ovabrw_dropoff_date'] ) ? sanitize_text_field( $_GET['ovabrw_dropoff_date'] ) : '';
...
if(  ( $rental_type == 'day' || $rental_type == 'hour' || $rental_type == 'mixed' ) && $ovabrw_pickup_date != '' && $ovabrw_dropoff_date != '' && apply_filters( 'ova_brw_finish_price_search', true ) ){

    $total_price =  get_price_by_date( $pid, strtotime( $ovabrw_pickup_date ), strtotime( $ovabrw_dropoff_date )); 

    $new_input_date = ovabrw_new_input_date( $pid, strtotime( $ovabrw_pickup_date ), strtotime( $ovabrw_dropoff_date ) );
    $real_quantity = get_real_quantity( 1, $pid, $new_input_date['pickup_date_new'], $new_input_date['dropoff_date_new'] );

    ?>

    <p class="<?php echo esc_attr( apply_filters( 'woocommerce_product_price_class', 'ovabrw-price price' ) );?>">
        <span class="amount"><?php echo wc_price( $total_price['line_total'] ); ?></span>
...
<?php }else if( $rental_type == 'period_time' || $rental_type == 'transportation' ) { ?>
        ...
        <?php if ( $min && $max && $min == $max ): ?>
            <span class="amount"><?php printf( __( '%s', 'ova-brw' ), wc_price( $min ) ); ?></span>
        <?php elseif ( $min && $max ): ?>
            <span class="amount"><?php printf( __( '%s - %s', 'ova-brw' ), wc_price( $min ), wc_price( $max ) ); ?></span>
        <?php else: ?>
