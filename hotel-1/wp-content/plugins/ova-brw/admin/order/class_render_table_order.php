...
class List_Booking extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $room_query = ( isset( $_POST['room_id'] ) && $_POST['room_id'] != '' ) ? 'AND order_item_meta.meta_value = '. sanitize_text_field( $_POST['room_id'] ) : '';
            ...
            if( ( empty( $order_number ) || ( ! empty( $order_number ) && $order_number == $value  ) ) && ( empty( $name_customer ) || ( strpos($fullname, $name_customer) ) !== false ) ) {
    	        ...
    	        foreach ( $order_line_items as $item_id => $item ) {
                    ...
                    if ( $item && is_object( $item ) ) {
                        ...
                        $room_check_out = date( $ovabrw_date_time_format, strtotime( $item->get_meta('ovabrw_dropoff_date') ) );
