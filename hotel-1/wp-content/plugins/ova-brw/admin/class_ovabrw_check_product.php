    ...
    <div class="wrap">
        <div class="booking_filter">
            <form id="booking-filter" method="GET" action="<?php echo admin_url('/edit.php?post_type=product&page=ovabrw-check-product'); ?>">
                <h2><?php esc_html_e( 'Check Room', 'ova-brw' ); ?></h2>
        		<select name="product_id">
        			<option value="" <?php selected( '', $current_product_id, 'selected'); ?>>
                        <?php esc_html_e( '-- Choose Room --', 'ova-brw' ); ?>
                    </option>
        			<?php 
                ...
                <div class="total_vehicle">
                    <?php esc_html_e( 'Total Room','ova-brw' ); ?>:
            ...
            <form id="available-vehicle" method="GET" action="<?php echo admin_url('/edit.php?post_type=product&page=check-product'); ?>" >
                <?php 
                    ...
                    if ( $product_id ) {
                        ...
                        $ova_validate_manage_store = ova_validate_manage_store( $product_id, $pickup_date_new, $dropoff_date_new, $current_pickup_loc, '', $passed = false, $validate = 'search', $number_vehicles ) ;
                ...
                ?>
                <h3><?php esc_html_e( 'The Available Room','ova-brw' ); ?></h3>
                    <select name="pickup_loc" style="display: none;">
                    ...
                    <button type="submit" class="button"><?php esc_html_e( 'Find Room', 'ova-brw' ); ?></button>
                    <?php if ( $vehicles_available ) { ?>
                    <table class="vehicle_id_available">
                        <thead>
                            <tr>
                                <td><strong><?php esc_html_e( 'Room ID', 'ova-brw' ); ?></strong></td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if ( $manage_vehicles == 'store' ): ?>
                                <tr>
                                    <td><?php echo esc_html__('Available Rooms: ', 'ova-brw') . $vehicles_available; ?></td>
                                </tr>
                            <?php else: ?>
                    ...
                    <?php } else { esc_html_e( 'Room not found','ova-brw' ); } ?>
                <!-- For plugins, we also need to ensure that the form posts back to our current page -->
                <input type="hidden" name="post_type" value="product" />
                <input type="hidden" name="page" value="<?php echo $_REQUEST['page'] ?>" />
            </form>
