    ...
    function get_order_rent_time( $product_id = false, $order_status = array( 'wc-completed' ) ) {
        ...
        foreach( $orders_ids as $key => $value ) {
            ...
            foreach( $order_line_items as $item_id => $item ) {
                ...
                $ovabrw_pickup_date_store   = $ovabrw_dropoff_date_store = $define_day = $rental_type = '';
                ...
                if ( in_array( $item->get_product_id(), $array_product_ids) ) {
                    ...
                    if ( $item && is_object( $item ) ) {
                        ...
                        $ovabrw_dropoff_date_store  = strtotime( $item->get_meta('ovabrw_dropoff_date_real') );
                        $ovabrw_number_vehicle      = trim( $item->get_meta('ovabrw_number_vehicle') );
                    }
                }

                // $ovabrw_dropoff_date_store - $ovabrw_pickup_date_store
                if ( $ovabrw_dropoff_date_store >= current_time( 'timestamp' ) ) {
                    for( $i = 0; $i < $ovabrw_number_vehicle ; $i++ ) { 
                        $push_date_unavailable = push_date_unavailable( $ovabrw_pickup_date_store, $ovabrw_dropoff_date_store, $define_day, $rental_type, $product_id );
...
if ( ! function_exists( 'push_date_unavailable' ) ) {
    function push_date_unavailable( $ovabrw_pickup_date_store = '', $ovabrw_dropoff_date_store = '', $define_day = '', $rental_type = '', $product_id = '' ) {
        ...
        $date_format = 'Y-m-d';
        $ovabrw_dropoff_date_store += $prepare_time;
        $date_time_pickup   = date( $date_format . ' H:i', $ovabrw_pickup_date_store );
        $date_time_pickoff  = date( $date_format . ' H:i', $ovabrw_dropoff_date_store );
        $date_pickup        = date( $date_format, $ovabrw_pickup_date_store );
        $date_pickoff       = date( $date_format, $ovabrw_dropoff_date_store );
        $time_pickup        = date( 'H:i', $ovabrw_pickup_date_store );
        $time_pickoff       = date( 'H:i', $ovabrw_dropoff_date_store );

        if ( $time_pickoff == '00:00' ) {
            $date_time_pickoff  = date( $date_format , $ovabrw_dropoff_date_store - 3600 * 24 ) . ' 24:00';
            $date_pickoff       = date( $date_format, $ovabrw_dropoff_date_store - 3600 * 24 );
            $time_pickoff       = '24:00';
        }
...
if ( ! function_exists( 'push_date_unavailable_time' ) ) {
    function push_date_unavailable_time( $ovabrw_pickup_date_untime, $ovabrw_dropoff_date_untime ) {
        $date_format = 'Y-m-d';
        $date_time_pickup   = date( $date_format . ' H:i', $ovabrw_pickup_date_untime );
        $date_time_pickoff  = date( $date_format . ' H:i', $ovabrw_dropoff_date_untime );
        $start_date     = date( $date_format, $ovabrw_pickup_date_untime );
        $end_date       = date( $date_format, $ovabrw_dropoff_date_untime );
        $time_pickup    = date( 'H:i', $ovabrw_pickup_date_untime );
        $time_pickoff   = date( 'H:i', $ovabrw_dropoff_date_untime );

        if ( $time_pickoff == '00:00' ) {
            $date_time_pickoff  = date( $date_format , $ovabrw_dropoff_date_untime - 3600 * 24 ) . ' 24:00';
            $end_date           = date( $date_format, $ovabrw_dropoff_date_untime - 3600 * 24 );
            $time_pickoff       = '24:00';
        }
...
if ( ! function_exists( 'ovabrw_search_vehicle' ) ) {
    function ovabrw_search_vehicle( $data_search ) {
        ...
        $dropoff_date   = isset( $data_search['ovabrw_dropoff_date'] )  ? strtotime( $data_search['ovabrw_dropoff_date'] ) : '';

        $order          = isset($data_search['order'])   ? sanitize_text_field( $data_search['order'] )     : 'ASC';
        $orderby        = isset($data_search['orderby']) ? sanitize_text_field( $data_search['orderby'] )   : 'title';
...
if ( ! function_exists( 'get_real_quantity' ) ) {
    function get_real_quantity( $product_quantity, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ) {
        if ( ! is_int( $ovabrw_pickup_date ) ) {
            $ovabrw_pickup_date = strtotime( $ovabrw_pickup_date );
        }
        if ( ! is_int( $ovabrw_dropoff_date ) ) {
            $ovabrw_dropoff_date = strtotime( $ovabrw_dropoff_date );
        }
        ...
        if ( $ovabrw_rt_startdate && apply_filters( 'ovabrw_ft_show_special_time', true ) ) {
            foreach ( $ovabrw_rt_startdate as $key_rt => $value_rt ) {
                ...
                // If Special_Time_Start_Date <= pickup_date && dropoff_date <= Special_Time_End_Date
                if ( $ovabrw_pickup_date >= strtotime( $start_date_str ) && $ovabrw_dropoff_date <= strtotime( $end_date_str ) ) {
                ...
                } elseif ( $ovabrw_pickup_date < strtotime( $start_date_str ) && $ovabrw_dropoff_date <= strtotime( $end_date_str ) && $ovabrw_dropoff_date >= strtotime( $start_date_str ) ) {
                    ...
                    $rt_quantity_array  = get_time_bew_2day( strtotime( $start_date_str ), $ovabrw_dropoff_date );
                ...
                } elseif ( $ovabrw_pickup_date >= strtotime( $start_date_str ) && $ovabrw_pickup_date <= strtotime( $end_date_str ) && $ovabrw_dropoff_date >= strtotime( $end_date_str ) ) {
                    $gl_quantity_array  = get_time_bew_2day( strtotime( $end_date_str ), $ovabrw_dropoff_date, $product_id );
                    $gl_quantity        = ovabrw_quantity_global( $product_id, $gl_quantity_array );
                ...
                } elseif ( $ovabrw_pickup_date < strtotime( $start_date_str ) && $ovabrw_dropoff_date > strtotime( $end_date_str ) ) {
                    // Time section 1
                    ...
                    $gl_rent_time_3 = get_time_bew_2day( strtotime( $end_date_str ), $ovabrw_dropoff_date );
...
if ( ! function_exists( 'get_real_price' ) ) {
    function get_real_price( $product_price, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ) {
        if ( ! is_int( $ovabrw_pickup_date ) ) {
            $ovabrw_pickup_date = strtotime( $ovabrw_pickup_date );
        }
        if ( ! is_int( $ovabrw_dropoff_date ) ) {
        ...
        if ( $price_type === "mixed" ) {
            $total_price_day    = get_price_by_rental_type_day( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
        ...
        if ( $ovabrw_rt_startdate && ( $price_type === 'day' || $price_type === 'hour' || $price_type === 'mixed' ) ) {
            foreach ( $ovabrw_rt_startdate as $key_rt => $value_rt ) {
                ...
                // If Special_Time_Start_Date <= pickup_date && dropoff_date <= Special_Time_End_Date
                if ( $ovabrw_pickup_date >= $start_date_str && $ovabrw_dropoff_date <= $end_date_str ) {
                ...
                } elseif ( $ovabrw_pickup_date < $start_date_str && $ovabrw_dropoff_date <= $end_date_str && $ovabrw_dropoff_date >= $start_date_str ) {
                    ...
                    $rt_quantity_array  = get_time_bew_2day( $start_date_str, $ovabrw_dropoff_date, $product_id );
                ...
                } elseif ( $ovabrw_pickup_date >= $start_date_str && $ovabrw_pickup_date <= $end_date_str && $ovabrw_dropoff_date >= $end_date_str ) {
                    $rt_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date, $end_date_str );
                    $rt_price           = ovabrw_price_special_time( $product_id, $rt_quantity_array, $key_rt );
                    $gl_quantity_array  = get_time_bew_2day( $end_date_str, $ovabrw_dropoff_date, $product_id );
                    $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
                    $product_price      .= wc_price( ovabrw_get_price_include_tax( $product_id, $rt_price ) ).' '.ovabrw_text_time_rt( $price_type, $rent_time ).'<br>';
                ...
                } elseif ( $ovabrw_pickup_date < $start_date_str && $ovabrw_dropoff_date > $end_date_str ) {
                    // Time section 1
                    $gl_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date, $start_date_str, $product_id );
                    $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
...
if ( ! function_exists( 'ovabrw_get_orders_not_remaining_invoice' ) ) {
    function ovabrw_get_orders_not_remaining_invoice() {
        ...
        $order_ids = $wpdb->get_col("
            ...
            LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
            WHERE posts.post_type = 'shop_order'
            AND order_item_meta.meta_key = 'ovabrw_remaining_amount_product'
...
if ( ! function_exists( 'ovabrw_get_html_resources' ) ) {
    function ovabrw_get_html_resources( $product_id = false, $resources = [], $start_date = false, $end_date = false ) {
        $html = '';
...
if ( ! function_exists( 'ovabrw_get_html_services' ) ) {
    function ovabrw_get_html_services( $product_id = false, $services = [], $start_date = false, $end_date = false ) {
        $html = '';
...
if ( ! function_exists( 'ovabrw_get_html_ckf' ) ) {
    function ovabrw_get_html_ckf( $custom_ckf = [] ) {
        $html = '';
            ...
            foreach ( $custom_ckf as $k => $val ) {
                if ( isset( $list_fields[$k] ) && ! empty( $list_fields[$k] ) ) {
                    ...
                    if ( $type === 'radio' ) {
                        ...
                        if ( ! is_bool( $val_key ) ) {
                            $price = $list_fields[$k]['ova_prices'][$val_key];

                            if ( $price ) {
                                $html .= '<dt>' . $val . esc_html__( ': ', 'ova-brw' ) . '</dt><dd>' . wc_price( $price ) . '</dd>';
                            }
                        }
                    }
                        ...
                        if ( ! empty( $val ) && is_array( $val ) ) {
                            foreach ( $val as $val_cb ) {
                                ...
                                if ( ! is_bool( $val_key ) ) {
                                    ...
                                    if ( $price ) {
                                        $html .= '<dt>' . $label . esc_html__( ': ', 'ova-brw' ) . '</dt><dd>' . wc_price( $price ) . '</dd>';
                                    }
                                }
                            }
                        }
    ...
    function ovabrw_get_html_resources_order( $id ) {
        ...
        if ( $resources ) {
            ...
            foreach ( $resources as $key => $value) {
                ...
                $rs_duration_type   = isset( $resources_duration_type[$key] ) ? $resources_duration_type[$key] : '';
...
if ( ! function_exists( 'ovabrw_check_table_price_by_day' ) ) {
    function ovabrw_check_table_price_by_day( $product_id ) {
        ...
        if ( ( $price_type == 'day' || $price_type == 'mixed' ) && $monday && $tuesday && $wednesday && $thursday && $friday && $saturday && $sunday && !empty( $daily ) ) { 
            $flag = true;
        }
...
if ( !function_exists( 'ovabrw_get_gallery_ids' ) ) {
    function ovabrw_get_gallery_ids( $product_id ) {
        $product = wc_get_product( $product_id );
...
/* Get Rooms */
if ( ! function_exists('ovabrw_get_room') ) {
    function ovabrw_get_room( $args ) {
        if ( ! $args ) {
            return false;
        }
        switch ( $args['orderby'] ) {
...
/* Get data option slider */
if ( ! function_exists('ovabrw_get_data_option') ) {
    function ovabrw_get_data_option($args) {
        $data_options = array();
        if ( $args ) {
