...
if ( ! function_exists( 'ovabrw_validation_booking_form' ) ) {
    function ovabrw_validation_booking_form( $passed ) {
        ...
        $ovabrw_dropoff_date    = strtotime( sanitize_text_field( filter_input( INPUT_POST, 'ovabrw_dropoff_date' ) ) );
        ...
        $ovabrw_adults      = (int)sanitize_text_field( filter_input( INPUT_POST, 'ovabrw_adults' ) );
        $ovabrw_childrens   = (int)sanitize_text_field( filter_input( INPUT_POST, 'ovabrw_childrens' ) );
        ...
        $max_adults     = (int)get_post_meta( $car_id, 'ovabrw_adult_number', true );
        $max_childrens  = (int)get_post_meta( $car_id, 'ovabrw_children_number', true );
        ...
        // Set Pick-up, Check-out Date again
        $new_input_date = ovabrw_new_input_date( $car_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $ovabrw_period_package_id, $ovabrw_pickup_loc, $ovabrw_pickoff_loc );
        ...
        if ( $pickup_date_new < current_time('timestamp') ) {
            wc_clear_notices();
            echo wc_add_notice( __("Check-in Date must be greater than Current Time", 'ova-brw'), 'error');   
            return false;
        }

        // Error adults, childrens
        if ( $ovabrw_adults > $max_adults ) {
            wc_clear_notices();
            echo wc_add_notice( sprintf( esc_html__( 'Please choose the number of adults less than %d', 'ova-brw' ), $max_adults ), 'error');
            return false;
        }

        if ( $ovabrw_adults < 1 ) {
            wc_clear_notices();
            echo wc_add_notice( __("Please choose the number of adults greater 0", 'ova-brw'), 'error');  
            return false;
        }

        if ( $ovabrw_childrens > $max_childrens ) {
            wc_clear_notices();
            echo wc_add_notice( sprintf( esc_html__( 'Please choose the number of childrens less than %d', 'ova-brw' ), $max_childrens ), 'error');
            return false;
        }
        ...
        if ( $rental_type !== 'transportation' ) {
            // Error empty Pick Up Date or Drop off Date
            if ( empty( $pickup_date_new ) || empty( $dropoff_date_new ) ) {
                wc_clear_notices();
                echo wc_add_notice( __("Insert Pick-up, Check-out Date", 'ova-brw'), 'error');
                return false;
            }

            // Error Pick Up Date > Drop off Date
            if ( $pickup_date_new >  $dropoff_date_new ) {
                wc_clear_notices();
                echo wc_add_notice( __("Check-out Date must be greater than Check-in Date", 'ova-brw'), 'error');
                return false;
            }
            ...
            if ( empty( $pickup_date_new ) ) {
                wc_clear_notices();
                echo wc_add_notice( __("Insert Check-in Date", 'ova-brw'), 'error');
                return false;
            }
        } 
        ...
        switch( $rental_type ) {
            case 'day':
                if ( !empty( $ovabrw_rent_day_max ) && ovabrw_val_day_max( $dropoff_date_new, $pickup_date_new, $ovabrw_rent_day_max ) ) {
                    wc_clear_notices();
                    echo wc_add_notice( sprintf( esc_html__( 'Max Rental Period: %d day', 'ova-brw' ), $ovabrw_rent_day_max ), 'error');   
                    return false;
                }

                if ( ovabrw_val_day_min( $dropoff_date_new, $pickup_date_new, $ovabrw_rent_day_min ) ) {
                    wc_clear_notices();
            ...
            case 'hour':
                if ( !empty( $ovabrw_rent_hour_max ) && ovabrw_val_hour_max( $dropoff_date_new, $pickup_date_new, $ovabrw_rent_hour_max ) ) {
                    wc_clear_notices();
                    echo wc_add_notice( sprintf( esc_html__( 'Max Rental Hours %d hour', 'ova-brw' ), $ovabrw_rent_hour_max ), 'error');   
                    return false;
                }

                if ( ovabrw_val_hour_min( $dropoff_date_new, $pickup_date_new, $ovabrw_rent_hour_min ) ) {
                    wc_clear_notices();
        ...
        $ova_validate_manage_store = ova_validate_manage_store( $car_id, $pickup_date_new, $dropoff_date_new, $ovabrw_pickup_loc, $ovabrw_pickoff_loc, $passed, $validate = 'cart', $ovabrw_number_vehicle );
...
if ( ! function_exists( 'ovabrw_add_extra_data_to_cart_item' ) ) {
    function ovabrw_add_extra_data_to_cart_item( $cart_item_data, $product_id, $variation_id ) {
        ...
        $ovabrw_dropoff_date    = sanitize_text_field( filter_input( INPUT_POST, 'ovabrw_dropoff_date' ) );
            ...
            foreach( $list_fields as $key => $field ) {
                if ( $field['enabled'] == 'on' ) {
                    if ( $field['type'] === 'file' ) {
                        ...
                        $val_checkbox = $_POST[$key] ? $_POST[$key] : '';
        ...
        if ( $price_type === 'transportation' ) {
            ...
            if ( $dropoff_date_by_setting != 'yes' ) {
                ...
                $ovabrw_dropoff_date    = date( $date_format . ' ' . $time_format, $dropoff_date_timestamp );
            }
        }
        ...
        if ( $price_type == 'period_time' ) {
            ...
            $ovabrw_dropoff_date = $rental_info_period['end_time'] ? date( $date_time_format, $rental_info_period['end_time'] ) : '';
        ...
        if ( ! $ovabrw_dropoff_date ) {
            $ovabrw_dropoff_date = $ovabrw_pickup_date;
        }

        // If all fields is empty
        if ( empty( $ovabrw_pickup_loc ) && empty( $ovabrw_pickoff_loc ) && empty( $ovabrw_pickup_date ) && empty( $ovabrw_dropoff_date ) ) {
        ...
        // Get Services
        $ovabrw_service = isset( $_POST['ovabrw_service'] ) ? $_POST['ovabrw_service'] : [];     
        ...
        //real date
        $real_date = ovabrw_get_real_date( $product_id, $price_type, $defined_one_day, $ovabrw_pickup_date, $ovabrw_dropoff_date );
        ...
        // add pick-up, Check-out Date that the guest choose
        $cart_item_data['ovabrw_pickup_date'] = $ovabrw_pickup_date;
        $cart_item_data['ovabrw_dropoff_date'] = $ovabrw_dropoff_date;

        // add real date to compare in search
        $cart_item_data['ovabrw_pickup_date_real'] = $real_date['pickup_date_real'];
        $cart_item_data['ovabrw_dropoff_date_real'] = $real_date['dropoff_date_real'];
...
if ( ! function_exists( 'ovabrw_display_extra_data_cart' ) ) {
    function ovabrw_display_extra_data_cart( $item_data, $cart_item ) {
        ...
        if ( empty( $cart_item['ovabrw_pickup_loc'] ) && empty( $cart_item['ovabrw_pickoff_loc'] ) && empty( $cart_item['ovabrw_pickup_date'] ) && empty( $cart_item['ovabrw_dropoff_date'] ) ) {
            wc_clear_notices();
        ...
        if ( isset( $cart_item['ovabrw_dropoff_date'] ) ) {
            if ( ( isset( $cart_item['define_day'] ) && $cart_item['define_day'] == 'hotel' ) || ( ! ovabrw_check_time_to_book( $cart_item['product_id'], 'end' ) ) ) {
                $date_time_format = $ovabrw_date_format;
            }

            $ovabrw_dropoff_date = date_i18n( $date_time_format, strtotime( $cart_item['ovabrw_dropoff_date'] ) );
            ...
            if ( $product_id ) {
                $show_dropoff_date = ovabrw_show_pick_date_product( $product_id, 'dropoff' );
            }

            if ( ! $show_dropoff_date ) {
                $item_data[] = array(
                    'key'     => esc_html__( 'Check-out Date', 'ova-brw' ),
                    'value'   => wc_clean( $ovabrw_dropoff_date ),
                    'display' => '',
                    'hidden'  => true,
                );
            } else {
        ...
        if ( isset( $cart_item['ovabrw_adults'] ) ){
            $item_data[] = array(
        ...
        if ( isset( $cart_item['ovabrw_childrens'] ) ){
            $item_data[] = array(
        ...
        if ( isset( $cart_item['resources'] ) && $cart_item['resources'] ) {
            if ( count( $cart_item['resources'] ) == 1 ) {
                $item_data[] = array(
                    ...
                    'value'   => wc_clean( join( ', ', $cart_item['resources'] ) ),
                    'display' => '',
                );   
            } else {
                $item_data[] = array(
...
if ( ! function_exists( 'ovabrw_after_checkout_validation' ) ) {
    function ovabrw_after_checkout_validation( $data, $errors ) {
        foreach( WC()->cart->get_cart() as $cart_item ) {
            ...
            $dropoff_date   = isset( $cart_item['ovabrw_dropoff_date'] ) ?  strtotime( $cart_item['ovabrw_dropoff_date'] ) : '';
            ...
            if ( !empty( $product ) && $product->is_type( 'ovabrw_car_rental' ) ) {
                ...
                // Set Pick-up, Check-out Date again
                $new_input_date = ovabrw_new_input_date( $car_id, $pickup_date, $dropoff_date, '', $pickup_loc, $pickoff_loc );
                $pickup_date_new = $new_input_date['pickup_date_new'];
                $dropoff_date_new = $new_input_date['dropoff_date_new'];

                $ova_validate_manage_store = ova_validate_manage_store( $car_id, $pickup_date_new, $dropoff_date_new, $pickup_loc, $pickoff_loc, $passed = true, $validate = 'checkout', $ovabrw_number_vehicle );
...
if ( ! function_exists( 'ovabrw_add_extra_data_to_order_items' ) ) {
    function ovabrw_add_extra_data_to_order_items( $item, $cart_item_key, $values, $order ) {
        ...
        if ( empty( $values['ovabrw_pickup_loc'] ) && empty( $values['ovabrw_pickoff_loc'] ) && empty( $values['ovabrw_pickup_date'] ) && empty( $values['ovabrw_dropoff_date'] ) ) {
            return;
        }
        ...
        $item->add_meta_data( 'ovabrw_dropoff_date', $values['ovabrw_dropoff_date'] );

        $item->add_meta_data( 'ovabrw_pickup_date_real', $values['ovabrw_pickup_date_real']);
        $item->add_meta_data( 'ovabrw_dropoff_date_real', $values['ovabrw_dropoff_date_real'] );
        ...
        if ( ( $ovabrw_rental_type == 'period_time' ) ) {
            ...
            $item->add_meta_data( 'ovabrw_price_detail', strip_tags( wc_price( $values['period_price'] ) ) );
        } else {
            if ( $ovabrw_rental_type == 'transportation' ) {
                $item->add_meta_data( 'ovabrw_total_days', intval(1) );
                $item->add_meta_data( 'ovabrw_price_detail', wc_price( $values['price_transport'] ) );
            } else {
                $real_quantity = get_real_quantity( 1, $product_id, strtotime( $values['ovabrw_pickup_date'] ), strtotime( $values['ovabrw_dropoff_date'] ) );
                $item->add_meta_data( 'ovabrw_total_days', $real_quantity );

                $real_price = get_real_price( 1, $product_id, strtotime( $values['ovabrw_pickup_date'] ), strtotime( $values['ovabrw_dropoff_date'] ) );
        ...
        if ( isset( $values['ovabrw_service'] ) && $values['ovabrw_service'] ) {
            ...
            $values['ovabrw_service'] = $ovabrw_service;
        ...
        if ( $values['resources'] ) {
            if ( count( $values['resources'] ) == 1 ) {
                $item->add_meta_data( esc_html__( 'Resource', 'ova-brw' ), join( ', ', $values['resources'] ) );
            } else {
                $item->add_meta_data( esc_html__( 'Resources', 'ova-brw' ), join( ', ', $values['resources'] ) ); 
            }
        ...
        if( isset( $values['ovabrw_adults'] ) ){
            $item->add_meta_data( 'ovabrw_adults', $values['ovabrw_adults'] );
        }

        if( isset( $values['ovabrw_childrens'] ) ){
...
if ( ! function_exists( 'ovabrw_val_day_max' ) ) {
    function ovabrw_val_day_max( $ovabrw_dropoff_date, $ovabrw_pickup_date, $ovabrw_rent_day_max ) {
        return ( $ovabrw_dropoff_date - $ovabrw_pickup_date ) > $ovabrw_rent_day_max*24*60*60 ? true : false;
    }
}

// Validate Rent Hour Max
if ( ! function_exists( 'ovabrw_val_hour_max' ) ) {
    function ovabrw_val_hour_max( $ovabrw_dropoff_date, $ovabrw_pickup_date, $ovabrw_rent_hour_max ) {
        return ( $ovabrw_dropoff_date - $ovabrw_pickup_date ) > $ovabrw_rent_hour_max*60*60 ? true : false;
    }
}

// Validate Rent Day Min
if ( ! function_exists( 'ovabrw_val_day_min' ) ) {
    function ovabrw_val_day_min( $ovabrw_dropoff_date, $ovabrw_pickup_date, $ovabrw_rent_day_min ) {
        return ( $ovabrw_dropoff_date - $ovabrw_pickup_date ) < $ovabrw_rent_day_min*24*60*60 ? true : false;
    }
}

// Validate Rent Hour Min
if ( ! function_exists( 'ovabrw_val_hour_min' ) ) {
    function ovabrw_val_hour_min( $ovabrw_dropoff_date, $ovabrw_pickup_date, $ovabrw_rent_hour_min ) {
        return ( $ovabrw_dropoff_date - $ovabrw_pickup_date ) < $ovabrw_rent_hour_min*60*60 ? true : false;
    }
}
...
if ( ! function_exists( 'ova_calculate_deposit_remaining_amount' ) ) {
    function ova_calculate_deposit_remaining_amount ( $cart_item ) {
        ...
        $ovabrw_dropoff_date    = strtotime( $cart_item['ovabrw_dropoff_date'] );
        $rent_time              = get_time_bew_2day( $ovabrw_pickup_date, $ovabrw_dropoff_date, $product_id );
        ...
        if ( isset( $cart_item['rental_type'] ) && $cart_item['rental_type'] == 'period_time' ) {
            ...
            if ( $cart_item['resources'] && is_array( $cart_item['resources'] ) ) {
                ...
                $line_total += ovabrw_get_total_resoure( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $cart_re );
            }

            if ( $cart_item['ovabrw_service'] && is_array( $cart_item['ovabrw_service'] ) ) {
                $line_total += ovabrw_get_total_service( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $cart_item['ovabrw_service'] );
            }
        } else {
            $get_price_by_date  = get_price_by_date( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
 ...
 * @param  [strtotime]  $ovabrw_dropoff_date   the date has been filtered via function
...
if ( ! function_exists( 'ova_validate_manage_store' ) ) {
    function ova_validate_manage_store( $car_id, $pickup_date, $dropoff_date, $pickup_loc, $pickoff_loc, $passed,  $validate = 'cart', $number_vehicle = 1 ) {
        ...
        if ( $untime_startdate ) {
            foreach( $untime_startdate as $key => $value ) {
                if ( !( $dropoff_date < strtotime( $untime_startdate[$key] ) || strtotime( $untime_enddate[$key] ) < $pickup_date ) ) {
                    if ( $validate != 'search' ) {
                        wc_clear_notices();
        ...
        $store_vehicle_rented = ovabrw_vehicle_rented_in_order( $car_id, $pickup_date, $dropoff_date );
        
        // Check Count Product in Cart
        $cart_vehicle_rented  = ovabrw_vehicle_rented_in_cart( $car_id, $validate, $pickup_date, $dropoff_date );
        
        // Vehicle availables
        $data_vehicle = ovabrw_get_vehicle_available( $car_id, $store_vehicle_rented, $cart_vehicle_rented, $number_vehicle, $manage_store, $pickup_date, $dropoff_date, $pickup_loc, $pickoff_loc, $passed, $validate );
...
if ( ! function_exists( 'ovabrw_vehicle_rented_in_order' ) ) {
    function ovabrw_vehicle_rented_in_order( $car_id, $pickup_date, $dropoff_date ) {
        ...
        if ( $orders_ids ) {
            foreach( $orders_ids as $key => $value ) {
                ...
                foreach( $order_line_items as $item_id => $item ) {
                    $pickup_date_store = $dropoff_date_store = $id_vehicle_rented = '';
                    ...
                    if ( in_array( $product_id , $array_product_ids ) ) {
                        ...
                        if ( $item && is_object( $item ) ) {
                            ...
                            $dropoff_date_store = strtotime( $item->get_meta('ovabrw_dropoff_date_real') ) + $prepare_time;
                        ...
                        if ( $dropoff_date_store >= current_time( 'timestamp' ) ) {
                            if ( ! ( $pickup_date >= $dropoff_date_store || $dropoff_date <= $pickup_date_store ) ) {
                                $quantity += $qty_order;
...
if ( ! function_exists( 'ovabrw_vehicle_rented_in_cart' ) ) {
    function ovabrw_vehicle_rented_in_cart( $car_id, $validate, $pickup_date, $dropoff_date ) {
        ...
        if ( $validate == 'cart' ) {
            foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
                ...
                if ( in_array( $product_id, $array_product_ids ) ) {
                    ...
                    if ( isset( $cart_item['ovabrw_pickup_date_real'] ) && isset( $cart_item['ovabrw_dropoff_date_real'] ) ) {
                        if ( !( $pickup_date >= strtotime( $cart_item['ovabrw_dropoff_date_real'] )+$prepare_time || $dropoff_date <= strtotime( $cart_item['ovabrw_pickup_date_real'] ) ) ) {
...
if ( ! function_exists( 'ovabrw_get_vehicle_available' ) ) {
    function ovabrw_get_vehicle_available( $car_id = false, $store_vehicle_rented = [], $cart_vehicle_rented = [], $number_vehicle = 1, $manage_store = '', $pickup_date = '', $dropoff_date = '', $pickup_loc = '', $pickoff_loc = '', $passed = false, $validate = 'cart' ) {
                ...
                foreach( $ids_vehicle_available as $key => $value ) {
                    ...
                    if ( isset($vehicle_avai['require_loc']) && $vehicle_avai['require_loc'] == 'yes' ) {
                        if ( $pickup_loc == $vehicle_avai['loc'] ) {
                            if ( !( $pickup_date >= strtotime( $id_vehicle_untime_enddate ) || $dropoff_date <= strtotime( $id_vehicle_untime_startday ) ) && $id_vehicle_untime_startday != '' && $id_vehicle_untime_enddate != '' ) {
...
if ( ! function_exists( 'ovabrw_new_input_date' ) ) {
    function ovabrw_new_input_date( $product_id = '', $pickup_date = '', $dropoff_date = '', $period_package = '', $pick_up_loc = '', $pick_off_loc = '' ) {

        if ( !$product_id ) return array( 'pickup_date_new' => '', 'dropoff_date_new' => '' );
        ...
        if ( $rental_type == 'period_time' && $period_package != '' && $pickup_date != '' ) {
            ...
            $dropoff_date       = $rental_time_period['end_time'];
        }
        ...
        if ( $rental_type == 'day' && $defined_one_day == 'day' ) {
            ...
            $dropoff_date   = $dropoff_date ? strtotime( date( apply_filters( 'ovabrw_new_dropoff_date_day_day' , 'Y-m-d H:i' ), $dropoff_date ) ) : '';
            ...
            $dropoff_date   = $dropoff_date ? strtotime( date( apply_filters( 'ovabrw_new_dropoff_date_day_hour' , 'Y-m-d H:i' ), $dropoff_date ) ) : '';
            ...
            $dropoff_date   = $dropoff_date ? strtotime( date( 'Y-m-d', $dropoff_date ) .' '.apply_filters( 'brw_real_dropoff_time_hotel', '11:00' ) ) : '';
        }
        ...
        if ( $rental_type === 'transportation' && $pick_up_loc != '' && $pick_off_loc != '' ) {
            ...
            if ( $dropoff_date_by_setting === 'yes' ) {
                $dropoff_date   = $dropoff_date ? strtotime( date( apply_filters( 'ovabrw_new_dropoff_date_location' , 'Y-m-d H:i' ), $dropoff_date ) ) : '';
            } else {
                ...
                $dropoff_date = $pickup_date + $time_dropoff;
            }
        }
        ...
        return array( 'pickup_date_new' => $pickup_date, 'dropoff_date_new' => $dropoff_date );
    }
}
...
if ( ! function_exists( 'ovabrw_check_unavilable_time' ) ) {
    function ovabrw_check_unavilable_time( $product_id, $pickup_date, $dropoff_date ) {
