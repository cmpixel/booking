    ...
    function change_formatted_meta_data( $meta_data, $item ) {
        ...
        if ( method_exists( $item, 'get_product_id' ) ) {
            ...
            if ( ! $show_dropoff_date ) {
                array_push( $hide_fields, 'ovabrw_dropoff_date' );
                array_push( $hide_fields, 'ovabrw_dropoff_date_real' );
            }
        }
        // End
        ...
        if ( $cart_item['data']->is_type('ovabrw_car_rental') ) {
            if ( isset( $cart_item['rental_type'] ) && ( $cart_item['rental_type'] == 'period_time' ) ) {
                ...
                if ( $cart_item['rental_type'] === 'transportation' ) {
                    ...
                    return '× '.get_real_quantity( $product_quantity, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
                }
            }
        } else {
    ...
    function ovabrw_filter_woocommerce_cart_item_quantity( $product_quantity, $cart_item_key, $cart_item ) {
        if ( $cart_item['data']->is_type('ovabrw_car_rental') ) {
            ...
            if ( isset( $cart_item['rental_type'] ) && ( $cart_item['rental_type'] === 'period_time' ) ) {
                ...
                $product_id             = $cart_item['product_id'];
                $new_input_date         = ovabrw_new_input_date( $product_id, strtotime($cart_item['ovabrw_pickup_date']), strtotime($cart_item['ovabrw_dropoff_date']) );
                $ovabrw_pickup_date     = $new_input_date['pickup_date_new'];
                $ovabrw_dropoff_date    = $new_input_date['dropoff_date_new'];
    ...
    function ovabrw_woocommerce_checkout_cart_item_quantity( $product_quantity, $cart_item, $cart_item_key ) {
        if ( $cart_item['data']->is_type('ovabrw_car_rental') ) {
            ...
            if ( ( $ovabrw_rental_type === 'period_time' || $ovabrw_rental_type === 'transportation' ) ) {
                ...
                return '<span class="ovabrw_qty">'.get_real_quantity( $product_quantity, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ).'</span>';
            }
        } else {
            return $product_quantity;
        }
    ...
    function ovabrw_filter_woocommerce_cart_item_price( $product_price, $cart_item, $cart_item_key ) {
        ...
        if ( $cart_item['data']->is_type('ovabrw_car_rental') ) {
            ...
            if ( $ovabrw_rental_type === 'period_time' ) {
                ...
                $ovabrw_dropoff_date = isset( $cart_item['ovabrw_dropoff_date'] ) ? strtotime( $cart_item['ovabrw_dropoff_date'] ) : '';
                ...
                if ( $ovabrw_rental_type === 'transportation' ) {
                    ...
                    return $regular_price ? wc_price( ovabrw_get_price_include_tax( $product_id, $regular_price ) ) : 0;
                } else {
                    return get_real_price( $product_price, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
                }
            }     
        } else {
...
if ( ! function_exists( 'ovabrw_filter_woocommerce_cart_item_subtotal' ) ) {
    function ovabrw_filter_woocommerce_cart_item_subtotal( $product_price, $cart_item, $cart_item_key ) {
        ...
        if ( $cart_item['data']->is_type('ovabrw_car_rental') ) {
            ...
            // Get html Resources
            $resource_html = ovabrw_get_html_resources( $product_id, $cart_item['resources'], $ovabrw_pickup_date, $ovabrw_dropoff_date );

            // Get html Services
            $service_html  = ovabrw_get_html_services( $product_id, $cart_item['ovabrw_service'], $ovabrw_pickup_date, $ovabrw_dropoff_date );
    ...
    function ovabrw_woocommerce_order_item_quantity_html( $quantity, $item ) {
        ...
        if ( $product->is_type( 'ovabrw_car_rental' ) ) {
            if ( $item && is_object( $item ) ) {
                ...
                $ovabrw_dropoff_date = date_i18n( $ovabrw_date_time_format, strtotime( $item->get_meta('ovabrw_dropoff_date') ) );
...
if ( ! function_exists( 'ovabrw_change_order_item_meta_title' ) ) {
    function ovabrw_change_order_item_meta_title( $key, $meta, $item ) {
        ...
        if ( 'ovabrw_dropoff_date' === $meta->key ) { 

            $ovabrw_dropoff_date = $meta->value;
            $meta->value = date( $ovabrw_date_time_format, strtotime( $ovabrw_dropoff_date ) );

            if( $defined_one_day == 'hotel' ) {
                $meta->value = date( $ovabrw_date_format, strtotime( $ovabrw_dropoff_date ) );
            }
        ...
        if ( is_admin() ) {
            if ( 'ovabrw_pickup_date_real' === $meta->key ) { 
                $key = esc_html__(' Check-in Date real', 'ova-brw');
            }
            if ( 'ovabrw_dropoff_date_real' === $meta->key ) { 
                $key = esc_html__(' Check-out Date real ', 'ova-brw'); 
            }
...
if ( ! function_exists( 'change_order_item_meta_value' ) ) {
    function change_order_item_meta_value( $value, $meta, $item ) {
        ...
        if ( 'ovabrw_dropoff_date' === $meta->key ) { $key = esc_html__(' Check-out Date  ', 'ova-brw'); }
        if ( 'ovabrw_total_days' === $meta->key ) { $key = esc_html__(' Total Time ', 'ova-brw'); }
        if ( 'ovabrw_distance' === $meta->key ) { $key = esc_html__(' Distance ', 'ova-brw'); }
        if ( 'ovabrw_extra_time' === $meta->key ) { $key = esc_html__(' Extra Time ', 'ova-brw'); }
        if ( 'ovabrw_duration' === $meta->key ) { $key = esc_html__(' Duration ', 'ova-brw'); }
...
if ( ! function_exists( 'ovarbrw_woocommerce_loop_product_link' ) ) {
    function ovarbrw_woocommerce_loop_product_link( $product_link ) {
        if ( isset( $_GET['ovabrw_search'] ) ) {
            ...
            if ( isset( $_GET['ovabrw_dropoff_date'] ) && $_GET['ovabrw_dropoff_date'] ) {
                $product_link = add_query_arg( 'dropoff_date', $_GET['ovabrw_dropoff_date'], $product_link );
            }
...
if ( ! function_exists( 'ovabrw_woocommerce_display_item_meta' ) ) {
    function ovabrw_woocommerce_display_item_meta( $html, $item, $args ) {
        ...
        foreach( $item->get_formatted_meta_data() as $meta_id => $meta ) {
            if ( in_array( $meta->key , apply_filters( 'ovabrw_order_detail_hide_fields', array( 'ovabrw_pickup_date_real' ,'ovabrw_dropoff_date_real', 'id_vehicle', 'define_day' ) ) )  ){
                $strings[] = '';
            } else {
        ...
        $html = str_replace('ovabrw_pickup_date', esc_html__(' Check-in Date ', 'ova-brw') , $html );
        $html = str_replace('ovabrw_dropoff_date', esc_html__(' Check-out Date ', 'ova-brw') , $html );
        ...
        $html = str_replace('ovabrw_adults', esc_html__(' Adults', 'ova-brw') , $html );
        $html = str_replace('ovabrw_childrens', esc_html__(' Childrens', 'ova-brw') , $html );
