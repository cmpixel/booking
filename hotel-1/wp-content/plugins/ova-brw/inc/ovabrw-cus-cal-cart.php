...
if ( ! function_exists( 'ovabrw_woocommerce_before_calculate_totals' ) ) {
    function ovabrw_woocommerce_before_calculate_totals( $cart_object ) {
        ...
        foreach( $cart_object->get_cart() as $cart_item_key => $cart_item) {
            ...
            $ovabrw_dropoff_date = isset( $cart_item['ovabrw_dropoff_date'] ) ? $cart_item['ovabrw_dropoff_date'] : '' ;
            ...
            $total = get_price_by_date( $product_id, strtotime( $ovabrw_pickup_date ), strtotime( $ovabrw_dropoff_date ), $cart_item );
...
if ( ! function_exists( 'ovabrw_get_total_resoure' ) ) {
    function ovabrw_get_total_resoure( $product_id = false, $ovabrw_pickup_date = false, $ovabrw_dropoff_date = false, $ovabrw_resource = [] ) {
                ...
                foreach( $resource_id as $key_id => $value_id ) {
                    if ( $key_c_re ==  $value_id ) {
                        if ( $resource_duration_type[$key_id] == 'total' ) {
                            ...
                            $total_price += $resource_price[$key_id] * $rent_time['rent_time_day'];
                        } elseif ( $resource_duration_type[$key_id] == 'hours' ) {
                            $total_price += $resource_price[$key_id] * $rent_time['rent_time_hour'];
                        }
                    }
                }
            }
...
if ( ! function_exists( 'ovabrw_get_total_service' ) ) {
    function ovabrw_get_total_service( $product_id = false, $ovabrw_pickup_date = false, $ovabrw_dropoff_date = false, $ovabrw_service = [] ) {
        $rent_time      = get_time_bew_2day( $ovabrw_pickup_date, $ovabrw_dropoff_date, $product_id );
                        ...
                        if ( in_array($id_service, $value_id_arr) ) {
                            foreach( $value_id_arr as $k => $v ) {
                                if ( $id_service == $v ) {
                                    ...
                                    } elseif ( $ovabrw_service_duration_type_str == 'days' ) {
                                        $total_service += $price_service * $rent_time['rent_time_day'];
                                    } elseif ( $ovabrw_service_duration_type_str == 'hours' ) {
                                        $total_service += $price_service * $rent_time['rent_time_hour'];
                                    }
                                }
                            }
                        }
 ...
 * @param  [strtotime] $ovabrw_dropoff_date
 * @return [array] quantity, line_total
 */
if ( ! function_exists( 'get_price_by_date' ) ) {
    function get_price_by_date( $product_id = false, $ovabrw_pickup_date = false, $ovabrw_dropoff_date = false, $cart_item = [], $ovabrw_pickup_loc = '', $ovabrw_pickoff_loc ='', $id_package = '' ) {
        ...
        $new_date               = ovabrw_new_input_date( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
        $ovabrw_pickup_date     = $new_date['pickup_date_new'];
        $ovabrw_dropoff_date    = $new_date['dropoff_date_new'];
        $rent_time              = get_time_bew_2day( $ovabrw_pickup_date, $ovabrw_dropoff_date, $product_id );
        ...
        if ( $rental_type == 'period_time' ) {
            ...
            $get_price_by_day   = get_price_by_rental_type_day( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
        ...
        if ( $rental_type === "mixed" || $rental_type === "hour" ) {
            ...
            if ( $ovabrw_rt_startdate ) {
                foreach( $ovabrw_rt_startdate as $key_rt => $value_rt ) {
                    ...
                    if ( $ovabrw_pickup_date >=  $start_date  && $ovabrw_dropoff_date <= $end_date ) {
                    ...
                    } elseif ( $ovabrw_pickup_date < $start_date && $ovabrw_dropoff_date <=  $end_date && $ovabrw_dropoff_date >=  $start_date ) {
                        ...
                        $rt_quantity_array  = get_time_bew_2day( $start_date, $ovabrw_dropoff_date, $product_id );
                    ...
                    } elseif ( $ovabrw_pickup_date >=  $start_date && $ovabrw_pickup_date <= $end_date && $ovabrw_dropoff_date >= $end_date ) {
                        $rt_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date,  $end_date, $product_id );
                        $rt_price           = ovabrw_price_special_time( $product_id, $rt_quantity_array, $key_rt );
                        $rt_quantity        = ovabrw_quantity_global( $product_id, $rt_quantity_array );
                        $gl_quantity_array  = get_time_bew_2day( $end_date, $ovabrw_dropoff_date, $product_id );
                        $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
                        $gl_quantity        = ovabrw_quantity_global( $product_id, $gl_quantity_array );
                    ...
                    } elseif ( $ovabrw_pickup_date < $start_date && $ovabrw_dropoff_date > $end_date ) {
                        ...
                        $gl_rent_time_3 = get_time_bew_2day( $end_date, $ovabrw_dropoff_date, $product_id );
        ...
        $total_resource = ovabrw_get_total_resoure( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $ovabrw_resource );
        $ovabrw_service = isset( $cart_item['ovabrw_service'] ) ? $cart_item['ovabrw_service'] : [];
        $total_service  = ovabrw_get_total_service( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $ovabrw_service );
...
if ( ! function_exists( 'get_total_price_special_by_type_price_day' ) ) {
    function get_total_price_special_by_type_price_day( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ) {
        $ovabrw_rt_startdate    = get_post_meta( $product_id, 'ovabrw_rt_startdate', true );
        $ovabrw_rt_starttime    = get_post_meta( $product_id, 'ovabrw_rt_starttime', true );
        $ovabrw_rt_enddate      = get_post_meta( $product_id, 'ovabrw_rt_enddate', true );
        $ovabrw_rt_endtime      = get_post_meta( $product_id, 'ovabrw_rt_endtime', true );
        ...
        if ( $ovabrw_rt_startdate ) {
            foreach( $ovabrw_rt_startdate as $key_rt => $value_rt ) {
                ...
                if ( $ovabrw_pickup_date >= $start_date && $ovabrw_dropoff_date <=  $end_date ) {
                    ...
                    $rent_time      = get_time_bew_2day( $start_date, $ovabrw_dropoff_date, $product_id );
                    $price_special  = ovabrw_price_special_time( $product_id, $rent_time, $key_rt );
...
if ( ! function_exists( 'get_total_price_special_by_type_price_day_sub' ) ) {
    function get_total_price_special_by_type_price_day_sub( $product_id = false, $ovabrw_pickup_date = false, $ovabrw_dropoff_date = false, $array_price_week = [], $weekday_start = null ) {
        $ovabrw_rt_startdate    = get_post_meta( $product_id, 'ovabrw_rt_startdate', true );
        $ovabrw_rt_starttime    = get_post_meta( $product_id, 'ovabrw_rt_starttime', true );
        $ovabrw_rt_enddate      = get_post_meta( $product_id, 'ovabrw_rt_enddate', true );
        $ovabrw_rt_endtime      = get_post_meta( $product_id, 'ovabrw_rt_endtime', true );
...
if ( ! function_exists( 'get_price_by_rental_type_day' ) ) {
    function get_price_by_rental_type_day( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ) {
        $ovabrw_pickup_date     = $ovabrw_pickup_date == '' ? null : $ovabrw_pickup_date;
        $ovabrw_dropoff_date    = $ovabrw_dropoff_date == '' ? null : $ovabrw_dropoff_date;
        ...
        $data_special_day   = get_total_price_special_by_type_price_day( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date );
        ...
        // Get total cost
        $price_special_day_sub = get_total_price_special_by_type_price_day_sub( $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date, $array_price_week, $weekstart );
...
if ( ! function_exists( 'get_real_price_detail' ) ) {
    function get_real_price_detail( $product_price, $product_id, $ovabrw_pickup_date, $ovabrw_dropoff_date ) {
        $rent_time  = get_time_bew_2day( $ovabrw_pickup_date, $ovabrw_dropoff_date, $product_id );
        $price_type = get_post_meta( $product_id, 'ovabrw_price_type', true );
        ...
        if ( $ovabrw_rt_startdate ) {
            foreach( $ovabrw_rt_startdate as $key_rt => $value_rt ) {
                // If Special_Time_Start_Date <= pickup_date && dropoff_date <= Special_Time_End_Date
                if ( $ovabrw_pickup_date >= strtotime( $ovabrw_rt_startdate[$key_rt] ) && $ovabrw_dropoff_date <= strtotime( $ovabrw_rt_enddate[$key_rt] ) ) {
                    $rt_price       = ovabrw_price_special_time( $product_id, $rent_time, $key_rt );
                    $product_price  =  $rt_price.' '.ovabrw_text_time_rt( $price_type, $rent_time );
                    break;
                } elseif ( $ovabrw_pickup_date < strtotime( $ovabrw_rt_startdate[$key_rt] ) && $ovabrw_dropoff_date <= strtotime( $ovabrw_rt_enddate[$key_rt] ) && $ovabrw_dropoff_date >= strtotime( $ovabrw_rt_startdate[$key_rt] ) ) {
                    $gl_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date, strtotime( $ovabrw_rt_startdate[$key_rt] ), $product_id );
                    $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
                    $rt_quantity_array  = get_time_bew_2day( strtotime( $ovabrw_rt_startdate[$key_rt] ), $ovabrw_dropoff_date, $product_id );
                    $rt_price           = ovabrw_price_special_time( $product_id, $rt_quantity_array, $key_rt );
                    $product_price      =  $gl_price .' '.ovabrw_text_time_gl( $price_type, $rent_time ).' ' . $rt_price  .' '.ovabrw_text_time_rt( $price_type, $rent_time );
                    break;
                } elseif ( $ovabrw_pickup_date >= strtotime( $ovabrw_rt_startdate[$key_rt] ) && $ovabrw_pickup_date <= strtotime( $ovabrw_rt_enddate[$key_rt] ) && $ovabrw_dropoff_date >= strtotime( $ovabrw_rt_enddate[$key_rt] ) ) {
                    $rt_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date, strtotime( $ovabrw_rt_enddate[$key_rt] ), $product_id );
                    $rt_price           = ovabrw_price_special_time( $product_id, $rt_quantity_array, $key_rt );
                    $gl_quantity_array  = get_time_bew_2day( strtotime( $ovabrw_rt_enddate[$key_rt] ), $ovabrw_dropoff_date, $product_id );
                    $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
                    $product_price      =  $gl_price . ' ' . ovabrw_text_time_gl( $price_type, $rent_time ) . ' ' . $rt_price . ' ' . ovabrw_text_time_rt( $price_type, $rent_time );
                    break;
                } elseif ( $ovabrw_pickup_date < strtotime( $ovabrw_rt_startdate[$key_rt] ) && $ovabrw_dropoff_date > strtotime( $ovabrw_rt_enddate[$key_rt] ) ) {
                    // Time section 1
                    $gl_quantity_array  = get_time_bew_2day( $ovabrw_pickup_date, strtotime( $ovabrw_rt_startdate[$key_rt] ), $product_id );
                    $gl_price           = ovabrw_price_global( $product_id, $gl_quantity_array );
                ...
                if ( $type === 'radio' && isset( $list_custom_ckf[$key]['ova_values'] ) && $list_custom_ckf[$key]['ova_values'] ) {
                    foreach ( $list_custom_ckf[$key]['ova_values'] as $k => $v ) {
                        if ( $val === $v && isset( $list_custom_ckf[$key]['ova_prices'][$k] ) ) {
                            $price += floatval( $list_custom_ckf[$key]['ova_prices'][$k] );
