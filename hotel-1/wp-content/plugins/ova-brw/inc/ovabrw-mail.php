...
if ( ! function_exists( 'ovabrw_mail_remind_event_time' ) ) {
	function ovabrw_mail_remind_event_time( $order, $customer_mail, $product_name, $product_id, $ovabrw_pickup_date ) {
		...
		$body 		= apply_filters( 'ovabrw_reminder_content_mail', ovabrw_get_setting( get_option( 'reminder_mail_content', esc_html__( 'You have hired a room: [ovabrw_vehicle_name] at [ovabrw_order_pickup_date]', 'ova-brw' ) ) ), $order );
